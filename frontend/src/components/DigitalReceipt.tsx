      ...
      <div className="bg-white text-slate-900 rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border-2 border-emerald-800/30 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h3 className="font-extrabold text-base tracking-tight leading-none">{t('receipt.title')}</h3>
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/20 font-bold uppercase tracking-wider">
                  {isCloudSynced ? t('common.verified') : t('common.offline')}
                </span>
              </div>
              <p className="text-[11px] text-emerald-100/80 mt-0.5">{t('receipt.govtHeader')}</p>
            </div>
          </div>
        ...
        <div className="p-5 space-y-4 text-xs">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                <div className="font-extrabold text-xs">
                  {isCloudSynced ? t('receipt.qrValid') : t('sync.walStatus')}
                </div>
                <div className="text-[11px] opacity-80">
                  {t('common.status')}: <span className="font-mono font-bold">{data.current_state}</span>
                </div>
              </div>
            </div>
            <span className={`px-2 py-1 rounded text-[10px] font-black uppercase tracking-wider ${
              isCloudSynced ? 'bg-emerald-200 text-emerald-950' : 'bg-amber-200 text-amber-950'
            }`}>
              {data.sync_status}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3 rounded-xl border border-slate-200">
            <div>
              <span className="text-[10px] uppercase font-bold text-slate-500">{t('farmer.activeToken')}</span>
              <div className="font-mono font-bold text-slate-900 text-xs truncate select-all">{data.transaction_id}</div>
            </div>
            <div>
              <span className="text-[10px] uppercase font-bold text-slate-500">{t('queue.farmer')}</span>
              <div className="font-bold text-slate-900 text-xs truncate">
                {data.farmer_name || `Farmer #${data.farmer_id}`}
              </div>
            </div>
            <div>
              <span className="text-[10px] uppercase font-bold text-slate-500">{t('farmer.targetMandi')}</span>
              <div className="font-bold text-slate-900 text-xs truncate">{data.mandi_name || `Mandi #${data.mandi_id}`}</div>
            </div>
            <div>
              <span className="text-[10px] uppercase font-bold text-slate-500">{t('billing.cropName')}</span>
              <div className="font-bold text-emerald-800 text-xs truncate">{data.crop_type || 'Wheat'}</div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <h4 className="font-bold text-slate-800 flex items-center space-x-1.5 text-xs">
              ...
              <span>{t('weighbridge.weightSummary')}</span>
            </h4>

            <div className="border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-100">
              <div className="flex justify-between p-2.5 bg-slate-50 font-medium text-slate-700">
                <span>{t('weighbridge.grossWeight')}</span>
                <span className="font-mono font-bold text-slate-900">
                  {data.gross_weight_qt ? `${data.gross_weight_qt.toFixed(2)} ${t('common.quintals')}` : '—'}
                </span>
              </div>
              <div className="flex justify-between p-2.5 bg-slate-50 font-medium text-slate-700">
                <span>{t('weighbridge.tareWeight')}</span>
                <span className="font-mono font-bold text-slate-900">
                  {data.tare_weight_qt ? `${data.tare_weight_qt.toFixed(2)} ${t('common.quintals')}` : '—'}
                </span>
              </div>
              <div className="flex justify-between p-2.5 bg-emerald-50/50 font-extrabold text-emerald-950">
                ...
                <span className="font-mono text-sm text-emerald-700 font-black">
                  {data.net_weight_qt ? `${data.net_weight_qt.toFixed(2)} ${t('common.quintals')}` : '—'}
                </span>
              </div>
              <div className="flex justify-between p-2.5 text-slate-700">
                <span>{t('billing.mspPrice')}</span>
                <span className="font-mono font-bold">
                  {data.rate_per_qt ? `₹${data.rate_per_qt.toFixed(2)} / ${t('common.quintals')}` : '—'}
                </span>
              </div>
              <div className="flex justify-between p-2.5 text-slate-700">
                <span>{t('billing.grossPayable')}</span>
                <span className="font-mono font-bold">
                  {data.gross_amount_inr ? `₹${data.gross_amount_inr.toFixed(2)}` : '—'}
                </span>
              </div>
              <div className="flex justify-between p-2.5 text-rose-700">
                <span>{t('billing.mandiDeductions')}</span>
                <span className="font-mono font-bold">
              ...
              <div className="flex justify-between p-3 bg-emerald-100/60 font-black text-emerald-950 text-sm">
                ...
                <span className="font-mono text-emerald-900 text-base font-black">
                  {data.invoice_amount_inr ? `₹${data.invoice_amount_inr.toFixed(2)}` : '—'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1 font-mono text-[10px] text-slate-600 select-all">
            <div className="flex items-center space-x-1 text-slate-800 font-bold font-sans text-xs mb-1">
              ...
              <span>{t('receipt.cryptoAuditTrail')}</span>
            </div>
            {data.payout_block_hash && (
              <div className="truncate">
                <span className="text-slate-400">{t('receipt.ledgerHash')}: </span>
                {data.payout_block_hash}
              </div>
            )}
            {data.token_signature && (
              <div className="truncate">
                <span className="text-slate-400">{t('receipt.hmacToken')}: </span>
                {data.token_signature}
              </div>
            )}
            {data.dbt_reference_id && (
              <div className="truncate">
                <span className="text-slate-400">{t('receipt.dbtRef')}: </span>
                {data.dbt_reference_id}
              </div>
            )}
          </div>
        ...
        <div className="p-4 bg-slate-100 border-t border-slate-200 flex justify-end space-x-2.5">
          <button
            onClick={handlePrint}
            className="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-800 border border-slate-300 rounded-xl text-xs font-bold flex items-center space-x-1.5 transition shadow-xs cursor-pointer"
          >
            <Printer className="w-4 h-4" />
            <span>{t('receipt.printSlip')}</span>
          </button>
          <button
            onClick={onClose}
            className="px-5 py-2 bg-emerald-700 hover:bg-emerald-600 text-white rounded-xl text-xs font-bold transition shadow-sm cursor-pointer"
          >
            {t('common.close')}
          </button>
        </div>
